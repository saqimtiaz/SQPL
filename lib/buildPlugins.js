 ...
 */
const clonePlugin = async (repository, name, callback) => {
	...
	try {
		...
		console.log(`  => Latest tag in repository: ${latestTag}`);

		await repoGit.checkout(latestTag);
		console.log(`  => Checked out tag ${latestTag} in ${tmpDir}`);
		callback(tmpDir);
	} catch (error) {
		console.error(`Error cloning or checking out repository: ${repository}`);
		console.error(error);
	}
};
